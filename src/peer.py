...
def re_distribute(sock, chunkhash):
    global download_duty
    global connected
    global CONNECT_NUM
    global ALL_GET
    ...
    if chosen_sender is not None:
        download_duty[chosen_sender].append(chunkhash)
        print(f'The {bytes.hex(chunkhash)} has been re-distribute to {chosen_sender}.')
        # If the chosen_sender is being used, add to the duty list.

        if not connected[chosen_sender]:
            # If the chosen_sender is rest, transmit the chunk at once.
            print(f'The {chosen_sender} begin to work at once.')
            connected[chosen_sender] = True
            get_header = struct.pack("HBBHHII", socket.htons(52305), TEAM_NUM, 2, socket.htons(HEADER_LEN),
                                     socket.htons(HEADER_LEN + len(chunkhash)), socket.htonl(0),
                                     socket.htonl(0))
            get_pkt = get_header + chunkhash
            initialize_receiver(from_addr=chosen_sender, cur_gethash=chunkhash)
            CONNECT_NUM = CONNECT_NUM + 1
            ALL_GET = False
            sock.sendto(get_pkt, chosen_sender)
    else:
        print("The distribution goes wrong. Because we cannot find any sender.")
    ...
    if Type == 0:  # Received an WHOHAS packet
        ...
        if len(ex_received_chunk[current_chunkhash]) < CHUNK_DATA_SIZE:
            ...
            ack_pkt = struct.pack("HBBHHII", socket.htons(52305), TEAM_NUM, 4, socket.htons(HEADER_LEN),
                                  socket.htons(HEADER_LEN + len(current_chunkhash)), socket.htonl(to_seq_num), socket.htonl(to_ack_num))
        ...
        if len(ex_received_chunk[current_chunkhash]) == CHUNK_DATA_SIZE:
            ...
            if from_addr in byte_to_receive:
                ...
                if cur_index < (len(download_duty[from_addr]) - 1):
                    ...
                    last_received[from_addr] = 0
                    CONNECT_NUM = CONNECT_NUM - 1
                    print("All the work of this sender is done, disconnect this sender.")
        ...
        if ack_num * MAX_PAYLOAD >= CHUNK_DATA_SIZE:
            ...
            if last_byte_acked[from_addr] + 1 <= seq_num <= last_byte_sent[from_addr]:
                ...
                if if_dup_seq_num in send_time_dict[from_addr].keys() and if_dup_seq_num > 0:
            ...
            print(f'The num_to_send is {num_to_send}.')
            if num_to_send > 0:
...
def crash_check(sock, config):
    ...
    for p in config.peers:
        ...
        if not connected[target_addr]:
            continue
        if crashed[target_addr]:
            continue
        if current_time - last_time[target_addr] > 15.6:
            ...
            ex_received_chunk[bytes.hex(current_chunkhash)] = bytes()  # clear the received chunk.
            ...
            for i in range(cur_index, len(download_duty[target_addr])):
                ...
                re_distribute(sock, target_chunkhash)
            ...
            for addr, value in send_time_dict.items():
                for seq, time_bf in value.items():
                    if time.time() - time_bf > timeout[addr]:
                        ...
                        for seq0 in send_time_dict[addr].keys():
                            left = (seq0 - 1) * MAX_PAYLOAD
                            right = min(seq0 * MAX_PAYLOAD, CHUNK_DATA_SIZE)
                            ...
                            send_time_dict[addr][seq0] = time.time()
